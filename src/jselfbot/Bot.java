...
public class Bot extends ListenerAdapter {
	...
	public Bot(Config config) {
		...
		emojis = new Emojis();
		todo = new Todolist();
		commands = new Command[] { new AvatarCmd(), new BashCmd(), new ClearCmd(), new DeleteCmd(emojis), new DiscrimCmd(),
				new EmbedCmd(), new EmoteCmd(), new EvalCmd(), new GameCmd(), new GoogleCmd(), new ListCmd(emojis), new MeCmd(),
				new PingCmd(), new PollCmd(), new QuoteCmd(), new SetCmd(emojis), new SpoilerCmd(),
				new TimeCmd(config.getZoneId()), new TodoCmd(todo), new LogCmd()};//, new LogFromCmd() 
	}

	@Override
	public void onReady(ReadyEvent event) {
		LoggerFactory.getLogger("Selfbot")
				.info("Successfully logged in as " + event.getJDA().getSelfUser().getName() + "#"
						+ event.getJDA().getSelfUser().getDiscriminator() + "!\nYou are currently in "
						+ event.getJDA().getGuilds().size() + " guilds!\nType " + prefix + "help in Discord to get started!");
	}
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if (event.getMessage().getContentRaw().toLowerCase().startsWith(prefixLower)) {
			String[] parts = event.getMessage().getContentRaw().substring(prefixLower.length()).trim().split("\\s+", 2);
			if (parts.length < 2)
			...
			if ("help".equalsIgnoreCase(parts[0])) {
				...
				builder.setAuthor(event.getJDA().getSelfUser().getName() + " Self-Help", null,
						event.getJDA().getSelfUser().getAvatarUrl() == null ? event.getJDA().getSelfUser().getDefaultAvatarUrl()
